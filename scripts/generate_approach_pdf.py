...
def main() -> None:
    ...
    story = [
        ...
        Paragraph(
            "The conversation layer is stateless over the full message history. It applies guardrails first, then classifies the turn into clarify, recommend, refine, or compare. Clarification is capped at two questions and only triggers when missing information would materially change the shortlist. When hosted credentials are present, an OpenAI-compatible LLM assists with structured state extraction and concise reply writing. If the hosted model is unavailable, the same local pipeline returns a valid schema-compliant response.",
            body,
        ),
    ...
    mean_candidate_hit = trace_metrics.get("mean_candidate_hit_rate", 0.0)
    ...
    metrics_table = Table(
        [
            ...
            ["Mean Recall@10", f"{mean_recall:.3f}"],
            ["Mean candidate hit rate", f"{mean_candidate_hit:.3f}"],
            ["Behavior probe pass rate", f"{pass_rate:.3f}"],
        ],
    ...
    story.extend(
        [
            ...
            Spacer(1, 0.15 * inch),
            Paragraph(
                "Evaluation is split into two layers. First, a replay harness walks the 10 public traces turn-by-turn, calls the same service entrypoint, validates schema compliance, verifies every returned URL is catalog-backed, and computes Recall@10 plus candidate hit rate. Second, targeted probes check hard behaviors that the replay traces under-sample: vague-query clarification, shortlist refinement, grounded product comparison, legal refusal, prompt-injection refusal, language constraints, and hallucination prevention.",
                body,
            ),
